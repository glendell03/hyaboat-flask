...
def run(
    ...
    while cap.isOpened():
        ...
        # print(f'fps: {fps_text} coords: {coords}')
        ...
        x_axis = height / 2
        y_axis = width / 2
        left_point = y_axis - coords["left"]
        right_point = coords["right"] - y_axis
        top_point = coords["top"]
        bottom_point = coords["bottom"]
        object_width = left_point + right_point
        object_center = object_width / 2

        print(
            f"top: {top_point}, bottom: {bottom_point}, x-axis: {x_axis}, y-axis: {y_axis}"
        )
